...
import utils.model as cls_model
import utils.subset_sampler as subsetsampler
import utils.calc_hr as calc_hr

parser = argparse.ArgumentParser(description="ReHash-cifar10-ClassSampler")
...
parser.add_argument('--arch', default='resnet50', choices=['resnet50', 'alexnet', 'vgg11', 'cls'], type=str,
...
parser.add_argument('--margin', default=0.4, type=float,
                    help='loss-parameter: margin (default: 0.8)')
...
def adsh_algo(code_length):
    ...
    '''
    if opt.arch == 'cls':
        model = cls_model.Res50_feature(class_num=10, code_len=code_length)
    else:
    ...
    if opt.adam:
        ...
        ignored_params = list(map(id, model.classifier.classifier.parameters()))
        base_params = filter(lambda p: id(p) not in ignored_params, model.parameters())
        optimizer = optim.SGD([
            {'params': base_params, 'lr': learning_rate},
            {'params': model.classifier.classifier.parameters(), 'lr': 5 * learning_rate},
    ...
    for iter in range(max_iter):
        ...
        for epoch in range(epochs):
            ...
            for iteration, (train_input, train_label, batch_ind) in tqdm(enumerate(trainloader)):
                ...
                train_label = train_label.cuda().squeeze()

                model.zero_grad()

                if opt.arch == 'cls':
                    pred, output = model(train_input)
                else:
                ...
                if opt.arch == 'cls':
                    loss_xent = xent_loss(pred, train_label)
                else:
                    loss_xent = torch.zeros(1).cuda()
                loss_hash, W = rh_loss(output, V, S, S_omega, V[batch_ind.cpu().numpy(), :], u_ind)

                loss = loss_xent + loss_hash
            ...
            logger.info('CLS_loss: {:.5f}'.format(loss_xent.item()))
            logger.info('Hash_loss: {:.5f}'.format(loss_hash.item()))
        ...
        Q = -2*code_length*Sim.cpu().numpy().transpose().dot(U) - 2 * gamma * barU + 2 * barWU
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/ReHash_cifar10_cls', opt.name, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
