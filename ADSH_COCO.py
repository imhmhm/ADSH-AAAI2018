...
parser.add_argument('--weight_rll', default=1.0, type=float,
                    help='loss-parameter: weight of rll (default: 1.0)')
...
def _dataset():
    ...
    dset_database = dp.DatasetProcessingMS_COCO(
        'data/coco', 'database_img.txt', 'database_label.txt', transformations
    )
    dset_test =  dp.DatasetProcessingMS_COCO(
        'data/coco', 'test_img.txt', 'test_label.txt', transformations
    )
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/ADSH', opt.name, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
