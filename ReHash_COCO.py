...
parser = argparse.ArgumentParser(description="ReHash-coco")
...
def _dataset():
    ...
    dset_database = dp.DatasetProcessingMS_COCO(
        'data/coco', 'database_img.txt', 'database_label.txt', transformations_train
    )
    dset_test =  dp.DatasetProcessingMS_COCO(
        'data/coco', 'test_img.txt', 'test_label.txt', transformations_test
    )
    ...
    databaselabels = load_label('database_label.txt', 'data/coco')
    testlabels = load_label('test_label.txt', 'data/coco')
...
def adsh_algo(code_length):
    ...
    weight_decay = 5e-4
    update_step = opt.update_step # [10, 20, 30, 40, 50]
    # update_step = [15, 30]
    ...
    for iter in range(max_iter):
        ...
        # _sampler= subsetsampler.ClassSampler(select_index, dset_database, batch_size, 4)
        ...
        for epoch in range(epochs):
            ...
            for iteration, (train_input, train_label, batch_ind) in tqdm(enumerate(trainloader)):
                ...
                loss, W = rh_loss(output, V, S, S_omega, V[batch_ind.cpu().numpy(), :], batch_ind)
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/coco/ReHash_coco', opt.name, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
